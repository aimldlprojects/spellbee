...
class SpellingTestApp:
    def __init__(self, root):
        ...
        self.selected_word_type = tk.StringVar()
        self.words_list = []
        
        # Getting the filter variables
        # self.selected_user = self.user_id.get()
        # self.selected_test_type = self.test_type.get()
        # self.selected_word_type = self.word_type.get()
        ...
        correct_words_filename = f"user_{self.selected_user}_correct_words.csv"
        incorrect_words_filename = f"user_{self.selected_user}_incorrect_words.csv"
        ...
        try:
            self.correct_words_df = pd.read_csv(correct_words_filename, names=cols)
        except FileNotFoundError:
            # If the file doesn't exist, there are no words to exclude
            self.correct_words_df = pd.DataFrame(columns=cols)
        try:
            self.incorrect_words_df = pd.read_csv(incorrect_words_filename, names=cols)
        except FileNotFoundError:
            # If the file doesn't exist, there are no words to exclude
    ...
    def update_words_based_on_user(self, event=None):
        ...
        if self.selected_user:
            ...
            self.user_id.current(self.default_user_index)
            self.words_df = self.words_df[(self.words_df['user'] == self.selected_user) | (self.words_df['user']=='User')]
    ...
    def update_words_based_on_test_type(self, event=None):
        ...
        if  self.selected_test_type:
            ...
            # attempted words - correct & incorrect practiced morethan a week ago 
                ...
                cols_to_keep = ['word', 'last_practiced']
                ...
                combined_df_sorted = combined_df.sort_values(by=['word', 'last_practiced'], ascending=[True, True])
                unique_words_df = combined_df_sorted.drop_duplicates(subset='word', keep='last')
                unique_words_df['last_practiced'] = pd.to_datetime(unique_words_df['last_practiced'])

                # Filter the words which are practised a week back
                week_ago = datetime.now() - timedelta(weeks=1)
                unique_words_df = unique_words_df[unique_words_df['last_practiced'] < week_ago]
    ...
    def update_word_length_options(self, event=None):
        ...
        #     pass
        # else:
        #     self.word_type['values'] = self.word_type_for_bhavi
        #     self.word_type.current(0)

        # if selected_user != 'User':
        #     words_df = words_df[(words_df['user'] == selected_user) | (words_df['user']=='User')]
        # else:
        #     words_df = words_df

        # if self.selected_word_type != "All":
        #     words_df = words_df[(words_df['word_type'] == self.selected_word_type)]
        # else:
        #     words_df = words_df
        ...
        # self.selected_lengths = ["All"] + unique_lengths_list

        # Selected_unique_lengths_list = [value for value in self.selected_lengths if value in unique_lengths_list]

        # if self.selected_lengths:
        #     if self.selected_lengths in unique_lengths_list:
        #         pass
        #     else:
        #         self.word_length['values'] = ["All"] + unique_lengths_list
        #         self.word_length.current(0)
        # else:
        #     self.word_length['values'] = ["All"] + unique_lengths_list
        #     self.word_length.current(0)           

    # def update_word_list_df(self, event=None):
    ...
    #     if self.selected_word_type and self.selected_word_type != "All":
    #         df = df[(df['word_type'] == self.selected_word_type)]
    #     else:
    #         df = df

    #     if selected_word_length and "All" not in selected_word_length:
    #         lengths = [int(length) for length in selected_word_length]
    #         df = df[df['word'].apply(len).isin(lengths)]
    #     else:
    #         df = df
    #     self.words_df = df
    ...
    def setup_gui(self):
        ...
        self.user_id = ttk.Combobox(self.test_options_frame, textvariable=self.selected_user, values=self.existing_user_ids, state="readonly")
        ...
        self.user_id.current(self.default_user_index)  # Set Bhavi as default if exists, otherwise first user
        ...
        # self.update_word_list_df()

        # Bind the update function to the combobox
        self.user_id.bind("<<ComboboxSelected>>", self.update_word_length_options)
        # self.user_id.bind("<<ComboboxSelected>>", self.update_word_length_options)
        # self.user_id.bind("<<ComboboxSelected>>", self.update_word_list_df)
        ...
        user_input_entry.bind('<Return>', self.check_spelling())
